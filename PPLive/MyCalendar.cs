...
namespace PPLive
{
    ...
    {
        #region 角度变换
        static double rad = 180 * 3600 / Math.PI;  //每弧度的角秒数
        static double RAD = 180 / Math.PI;         //每弧度的角度数
        ...
        /// <returns></returns>
        private double Floor(double input)
        {
            double ret = Math.Floor(input);
            if (ret == 0)
            {
                ret = ret + 1;
            }
        ...
        /// <param name="input"></param>
        /// <returns></returns>
        private double RadSharper(double input)
        {
            double ret = input % (2 * Math.PI);
            if (ret < 0)
            {
                ret = ret + 2 * Math.PI;
            }
        ...
        /// </summary>
        /// <param name="input"></param>
        /// <param name="type">0输出格式示例: -23°59' 48.23",1输出格式示例:  18h 29m 44.52s</param>
        /// <returns></returns>
        private string RadToStr(double input, int type)
        {
            ...
            int a = (int)Math.Floor(input);
            input = (input - a) * 60;
            int b = (int)Math.Floor(input);
            input = (input - b) * 60;
            int c = (int)Math.Floor(input);
            input = (input - c) * 100;
            int d = (int)Math.Floor(input + 0.5);
            while (d >= 100)
            {
                d -= 100; c++;
            }
            ...
            m_sb.Append(a).Append(s1);
            if (b < 10)
            {
                m_sb.Append("0").Append(b).Append(s2);
            }
            else
            {
                m_sb.Append(b).Append(s2);
            }
            if (c < 10)
            {
                m_sb.Append("0").Append(c).Append(".");
            }
            else
            {
                m_sb.Append(c).Append(".");
            }
            if (d < 10)
            {
                m_sb.Append("0").Append(d).Append(s3);
            }
            else
            {
                m_sb.Append(d).Append(s3);
            }
        ...
        #region 日历计算
        int J2000 = 2451545; //2000年前儒略日数(2000-1-1 12:00:00格林威治平时)
        //DateTime MyDate = new DateTime();

        //double MyDate.Year = 2000;
        //double MyDate.Month = 1;
        //double MyDate.Day = 1;
        //double MyDate.Hour = 12;
        //double MyDate.Minute = 0;
        //double MyDate.Second = 0;
        double[] dts =       //世界时与原子时之差计算表
        {
        ...
        /// <returns></returns>
        private double DeltatYear(double year)
        {
            ...
            double t2 = t1 * t1;
            double t3 = t2 * t1;
            ret = dts[i + 1] + dts[i + 2] * t1 + dts[i + 3] * t2 + dts[i + 4] * t3;
            return ret;
        }
        ...
        /// <returns></returns>
        private double DeltatTDay(double JDay)
        {
            return this.DeltatYear((JDay / 365.2425 + 2000)) / 86400.0;
        }
        ...
        /// <returns></returns>
        private double DayToJDay(bool UTC, DateTime tmpdate)
        {
            double ret = new double();
            double tmpMonth = tmpdate.Month;
            double tmpYear = tmpdate.Year;

            if (tmpMonth <= 2)
            {
                tmpMonth += 12; tmpYear--;
            }
            if (tmpYear * 372 + tmpMonth * 31 + tmpdate.Day >= 588829)//判断是否为格里高利历日1582*372+10*31+15
                ret = Floor(tmpYear / 100); ret = 2 - ret + Floor(ret / 4);//加百年闰
            ret += Floor(365.2500001 * (tmpYear + 4716));   //加上年引起的偏移日数
            ret += Floor(30.6 * (tmpMonth + 1)) + tmpdate.Day;       //加上月引起的偏移日数及日偏移数
            ret += ((tmpdate.Second / 60 + tmpdate.Minute) / 60 + tmpdate.Hour) / 24 - 1524.5;
            if (UTC)
            {
                ret = ret + DeltatTDay(ret - J2000);
            }
            return ret;
        }
        ...
        /// </summary>
        /// <param name="jd"></param>
        /// <param name="UTC">UTC=1表示目标公历是UTC</param>
        /// <returns></returns>
        private DateTime JDayToDay(double JDay, bool UTC)
        {
            double tmpYear;
            double tmpMonth;
            double tmpDay;
            ...
            if (UTC)
            {
                JDay -= DeltatTDay(JDay - J2000);
            }
            JDay += 0.5;
            double A = Floor(JDay), F = JDay - A, D;  //取得日数的整数部份A及小数部分F
            if (A > 2299161)
            {
            ...
            A += 1524; //向前移4年零2个月
            tmpYear = Floor((A - 122.1) / 365.25);//年
            D = A - Floor(365.25 * tmpYear); //去除整年日数后余下日数
            tmpMonth = Floor(D / 30.6001);       //月数
            tmpDay = D - Floor(tmpMonth * 30.6001);//去除整月日数后余下日数
            tmpYear -= 4716;
            tmpMonth--;
            if (tmpMonth > 12)
            {
            ...
            //日的小数转为时分秒
            F *= 24; tmpHour = Floor(F); F -= tmpHour;
            F *= 60; tmpMinute = Floor(F); F -= tmpMinute;
            F *= 60; tmpSecond = F;
            return new DateTime((int)tmpYear, (int)tmpMonth, (int)tmpDay).AddHours((int)tmpHour).AddMinutes((int)tmpMinute).AddSeconds((int)tmpSecond);
        }

        /// <summary>
        /// 字符串转时间
        /// </summary>
        /// <param name="str">"20000101 120000"或"20000101"</param>
        private DateTime StrToDate(string str)
        {
            ...
            tmpYear = double.Parse(str.Substring(0, 4));
            tmpMonth = double.Parse(str.Substring(4, 2));
            tmpDay = double.Parse(str.Substring(6, 2));
            try
            {
                tmpHour = double.Parse(str.Substring(9, 2));
            }
            catch
            {
                tmpHour = 0;
            ...
            try
            {
                tmpMinute = double.Parse(str.Substring(11, 2));
            }
            catch
            {
                tmpMinute = 0;
            ...
            try
            {
                tmpSecond = double.Parse(str.Substring(13, 5));
            }
            catch
            {
                tmpSecond = 0;
            }
            return new DateTime((int)tmpYear, (int)tmpMonth, (int)tmpDay, (int)tmpHour, (int)tmpMinute, (int)tmpSecond);
        }
        ...
        /// </summary>
        /// <returns></returns>
        private string DateToStr(DateTime tmpdate)
        {
            StringBuilder sb = new StringBuilder();
            double h = tmpdate.Hour;
            double m = tmpdate.Minute;
            double s = Math.Floor(tmpdate.Second + 0.5);
            if (s >= 60)
            {
                s -= 60;
                m++;
            ...
            sb.Append(((int)tmpdate.Year).ToString()).Append("-");
            if (tmpdate.Month < 10)
            {
                sb.Append("0");
            }
            sb.Append(((int)tmpdate.Month).ToString()).Append("-");
            if (tmpdate.Day < 10)
            {
                sb.Append("0");
            }
            sb.Append(((int)tmpdate.Day).ToString()).Append(" ");
            if (h < 10)
            {
                sb.Append("0");
            }
            sb.Append(((int)h).ToString()).Append(":");
            if (m < 10)
            {
                sb.Append("0");
            }
            sb.Append(((int)m).ToString()).Append(" ");
            if (s < 10)
            {
                sb.Append("0");
            }
            sb.Append(((int)s).ToString());
        ...
        /// </summary>
        /// <param name="JDay"></param>
        /// <param name="shiqu">时区,北京的起算点是-8小时,shiqu取8</param>
        /// <param name="int_dec">1为整数</param>
        /// <returns>算出:jd转到当地UTC后,UTC日数的整数部分或小数部分</returns>
        private double LocalDayModify(double JDay, double shiqu, bool int_dec)
        {
            double ret = JDay + 0.5 - DeltatTDay(JDay) + shiqu / 24;
            if (int_dec)
            {
                ret = Math.Floor(ret); //返回整数部分
            }
            else
            {
                ret = ret - Math.Floor(ret);      //返回小数部分
            }
            return ret;
        }
        ...
        /// <returns></returns>
        private double GetDayDiff(DateTime day1, DateTime day2)
        {
            double ret = new double();
            day1 = new DateTime(day1.Year, day1.Month, day1.Day, 12, 0, 0);
            day2 = new DateTime(day2.Year, day2.Month, day2.Day, 12, 0, 0);
            double jd1 = DayToJDay(false, day1);
            double jd2 = DayToJDay(false, day2);
            if (jd1 > jd2)
            {
                ret = Math.Floor(jd1 - jd2 + .0001);
            }
            else
            {
                ret = Math.Floor(jd2 - jd1 + .0001);
            }
            return ret;
        }
        #endregion 日历计算
        ...
        #region 黄赤交角及黄赤坐标变换
        double[] hcjjB = { 84381.448, -46.8150, -0.00059, 0.001813 };//黄赤交角系数表
        ...
        /// <param name="t"></param>
        /// <returns></returns>
        private double GetHCAngle(double JDay)
        {
            double t1 = JDay / 36525;
            double t2 = t1 * t1;
            double t3 = t2 * t1;
        ...
        /// 黄赤转换(黄赤坐标旋转)
        /// </summary>
        /// <param name="JingWei"></param>
        /// <param name="E"></param>
        private void HCConvert(ref double[] JingWei, double E)
        {
            double HJ = RadSharper(JingWei[0]);
            double HW = JingWei[1];
            ...
            JingWei[0] = RadSharper(J);
            JingWei[1] = Math.Asin(sinW);
        }
        /// <summary>
        /// 补岁差
        /// </summary>
        /// <param name="JDay"></param>
        /// <param name="ZB"></param>
        private void AddPrece(double JDay, ref double[] ZB)
        {
            double t = 1, v = 0;
            double t1 = JDay / 365250;
            for (int i = 0; i < 8; i++)
            {
                t *= t1;
                v += preceB[i] * t;
                ZB[0] = RadSharper(ZB[0] + (v + 2.9965 * t1) / rad);
            }
        }
        #endregion 黄赤交角及黄赤坐标变换

        #region 光行差
        double[] GXC_e = { 0.016708634, -0.000042037, -0.0000001267 }; //离心率
        double[] GXC_p = { 102.93735 / RAD, 1.71946 / RAD, 0.00046 / RAD };  //近点
        double[] GXC_l = { 280.4664567 / RAD, 36000.76982779 / RAD, 0.0003032028 / RAD, 1 / 49931000 / RAD, -1 / 153000000 / RAD }; //太平黄经
        double GXC_k = 20.49552 / rad; //光行差常数
        ...
        /// <param name="JDay"></param>
        /// <param name="ZB"></param>
        private void AddGXC(double JDay, ref double[] ZB)
        {
            double t1 = JDay / 36525, t2 = t1 * t1, t3 = t2 * t1, t4 = t3 * t1;
        ...
        /// <param name="JDay"></param>
        /// <returns></returns>
        private double[] Nutation(double JDay)
        {
            double[] ret = new double[2];
            ret[0] = 0; ret[1] = 0;
            JDay /= 36535;
            double t1 = JDay, t2 = t1 * t1, t3 = t2 * t1, t4 = t3 * t1, t5 = t4 * t1, c;
            for (int i = 0; i < nutB.Length; i = i + 9)
            {
                c = nutB[i] + nutB[i + 1] * t1 + nutB[i + 2] * t2 + nutB[i + 3] * t3 + nutB[i + 4] * t4;
                ret[0] += (nutB[i + 5] + nutB[i + 6] * JDay / 10) * Math.Sin(c); //黄经章动
                ret[1] += (nutB[i + 7] + nutB[i + 8] * JDay / 10) * Math.Cos(c); //交角章动
            }
            ret[0] /= rad * 10000; //黄经章动
            ret[1] /= rad * 10000; //交角章动
            return ret;
        }
        ...
        /// <param name="JDay"></param>
        /// <param name="ZB"></param>
        private void NutationRaDec(double JDay, ref double[] ZB)
        {
            double Ra = ZB[0], Dec = ZB[1];
            double E = GetHCAngle(JDay), sinE = Math.Sin(E), cosE = Math.Cos(E); //计算黄赤交角
            double[] d = Nutation(JDay);                                  //计算黄经章动及交角章动
        ...
        double[] E13 = { 0.00000289226, 5.84384198723, 6283.0758499914, 0.00000034955, 0.00000000000, 0.0000000000, 0.00000016819, 5.48766912348, 12566.1516999828 };
        double[] E14 = { 0.00000114084, 3.14159265359, 0.0000000000, 0.00000007717, 4.13446589358, 6283.0758499914, 0.00000000765, 3.83803776214, 12566.1516999828 };
        double[] E15 = { 0.00000000878, 3.14159265359, 0.0000000000 };
        double[] E20 = { //黄纬周期项
        ...
        double[] E21 = { 0.00000009030, 3.89729061890, 5507.5532386674, 0.00000006177, 1.73038850355, 5223.6939198022 };
        double[] E30 = { //距离周期项
        ...
        double[] E31 = { 0.00103018608, 1.10748969588, 6283.0758499914, 0.00001721238, 1.06442301418, 12566.1516999828, 0.00000702215, 3.14159265359, 0.0000000000 };
        double[] E32 = { 0.00004359385, 5.78455133738, 6283.0758499914 };
        double[] E33 = { 0.00000144595, 4.27319435148, 6283.0758499914 };
        //月球运动参数
        double[] M10 = {
        ...
        double[] M12 = { 0.0048700, -0.0431256817, 628.3019552485, -2.6638815E-06, 6.1639211E-10, -5.4439728E-11, 0.0022800, -27.1705318325, -2.3011998397, 1.5231275E-04, 2.5041111E-07, -1.1863391E-09, 0.0015000, 11.2260974062, 6585.7609102104, -2.1583699E-04, -1.8708058E-07, 9.3204945E-10 };
        double[] M20 = {
        ...
        double[] M1n = {3.81034392032, 8.39968473021E+03, -3.31919929753E-05, //月球平黄经系数
                          3.20170955005E-08, -1.53637455544E-10};
        #endregion 月球及地球运动参数表

        #region 日位置计算
        double EnnT = 0; //调用Enn前先设置EnnT时间变量

        /// <summary>
        /// 计算E10,E11,E20等,即:某一组周期项或泊松项算出,计算前先设置EnnT时间
        /// </summary>
        /// <param name="tmpE"></param>
        /// <returns></returns>
        private double Enn(double[] tmpE)
        {
            double ret = 0;
            for (int i = 0; i < tmpE.Length; i = i + 3)
            {
                ret += tmpE[i] * Math.Cos(tmpE[i + 1] + EnnT * tmpE[i + 2]);
            }
            return ret;
        }
        ...
        /// <param name="JDay"></param>
        /// <returns></returns>
        private double[] EarthCal(double JDay)
        {
            EnnT = JDay / 365250;
            double[] ret = new double[3];
            double t1 = EnnT, t2 = t1 * t1, t3 = t2 * t1, t4 = t3 * t1, t5 = t4 * t1;
            ret[0] = Enn(E10) + Enn(E11) * t1 + Enn(E12) * t2 + Enn(E13) * t3 + Enn(E14) * t4 + Enn(E15) * t5;
            ret[1] = Enn(E20) + Enn(E21) * t1;
            ret[2] = Enn(E30) + Enn(E31) * t1 + Enn(E32) * t2 + Enn(E33) * t3;
            ret[0] = RadSharper(ret[0]);
            return ret;
        }
        ...
        /// <param name="JDay"></param>
        /// <returns></returns>
        private double[] SunCal(double JDay)
        {
            double[] sun = EarthCal(JDay);
            sun[0] += Math.PI; sun[1] = -sun[1]; //计算太阳真位置
            double[] d = Nutation(JDay);
            sun[0] = RadSharper(sun[0] + d[0]);   //补章动
            AddGXC(JDay, ref sun);  //补周年黄经光行差
            return sun;      //返回太阳视位置
        }
        #endregion 日位置计算

        #region 月位置计算
        double MnnT = 0; //调用Mnn前先设置MnnT时间变量
        ...
        /// <returns></returns>
        private double Mnn(double[] tmpM)
        {
            double ret = 0;
            double t1 = MnnT, t2 = t1 * t1, t3 = t2 * t1, t4 = t3 * t1;
            for (int i = 0; i < tmpM.Length; i = i + 6)
            {
                ret += tmpM[i] * Math.Sin(tmpM[i + 1] + t1 * tmpM[i + 2] + t2 * tmpM[i + 3] + t3 * tmpM[i + 4] + t4 * tmpM[i + 5]);
            }
            return ret;
        }
        ...
        /// <param name="JDay"></param>
        /// <returns></returns>
        private double[] MoonCal(double JDay)
        {
            MnnT = JDay / 36525;
            double t1 = MnnT;
            double t2 = t1 * t1, t3 = t2 * t1, t4 = t3 * t1;
            double[] ret = new double[3];
            ret[0] = (Mnn(M10) + Mnn(M11) * t1 + Mnn(M12) * t2) / rad;
            ret[1] = (Mnn(M20) + Mnn(M21) * t1) / rad;
            ret[2] = (Mnn(M30) + Mnn(M31) * t1) * 0.999999949827;
            ret[0] = ret[0] + M1n[0] + M1n[1] * t1 + M1n[2] * t2 + M1n[3] * t3 + M1n[4] * t4;
            ret[0] = RadSharper(ret[0]); //地心Date黄道原点坐标(不含岁差)
            AddPrece(JDay, ref ret); //补岁差
            return ret;
        }
        ...
        /// <param name="JDay"></param>
        /// <returns></returns>
        private double[] MoonCal2(double JDay)
        {
            double[] ret = MoonCal(JDay);
            double[] d = Nutation(JDay);
            ret[0] = RadSharper(ret[0] + d[0]); //补章动
            return ret;
        }
        ...
        /// <param name="JDay"></param>
        /// <returns></returns>
        private double[] MoonCal3(double JDay)
        {
            double[] ret = MoonCal(JDay);
            HCConvert(ref ret, GetHCAngle(JDay));
            NutationRaDec(JDay, ref ret); //补赤经及赤纬章动
        ...
        /// 
        /// </summary>
        /// <param name="lx">lx为真时计算JDay时刻日月角距与angle的差, lx为假计算JDay时刻太阳黄经与angle的差</param>
        /// <param name="JDay"></param>
        /// <param name="angle"></param>
        /// <returns></returns>
        private double AngleCai(bool lx, double JDay, double angle)
        {
            double ret;
            double[] sun = EarthCal(JDay);  //计算太阳真位置(先算出日心坐标中地球的位置)
            sun[0] += Math.PI; sun[1] = -sun[1]; //转为地心坐标
            AddGXC(JDay, ref sun);      //补周年光行差
            if (!lx)
            {
                double[] d = Nutation(JDay);
                sun[0] += d[0]; //补黄经章动
                ret = RadSharper(angle - sun[0]);
            }
            else
            {
                var moon = MoonCal(JDay); //日月角差与章动无关
                return RadSharper(angle - (moon[0] - sun[0]));
            }
            return ret;
        }
        #endregion 地心坐标中的日月位置计算

        #region 已知位置反求时间
        /// <summary>
        /// 已知角度(angle)求时间(t)
        /// 对于节气计算,应满足返回值在JDay到JDay+360天之间,对于Y年第n个节气(n=0是春分),JDay可取值Y*365.2422+n*15.2
        /// 对于朔望计算,应满足返回值在JDay到JDay+25天之间,在此范围之外,求右边的根
        /// </summary>
        /// <param name="JDay">定角度对应真时刻的前一些天，从J2000起算儒略日数</param>
        /// <param name="angle"></param>
        /// <param name="lx">lx为真假是太阳黄经达某角度的时刻计算(用于节气计算)，lx为真是日月角距达某角度的时刻计算(用于定朔望等)</param>
        /// <returns></returns>
        private double AngleCal(double JDay, double angle, bool lx)
        {

            double t2 = JDay, t = 0, v;
            if (!lx)

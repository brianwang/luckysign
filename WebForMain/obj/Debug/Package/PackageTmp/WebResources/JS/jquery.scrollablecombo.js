        ...
        return this.each(function () {
			...
			*/
            ...
            $this.find('option').each(function () {
                ...
                var $li_e = $('<li />', {
                    className: liclass,
                    html: '<a href="|' + id + '|' + this.index + '">' + $option.html() + '</a>'
                });
                $ul_e.append($li_e);
            });
            ...
            $ul_e.find('a').bind('click', function (e) {
                ...
                closeCombo();
                e.preventDefault();
                var tmp = this.href.split('%7C');
                document.getElementById(tmp[1]).options[tmp[2]].selected = true;
                if (document.getElementById(tmp[1]).fireEvent)
                    document.getElementById(tmp[1]).fireEvent("onchange");
                else
                    document.getElementById(tmp[1]).onchange();
            });
